...
import { CheckoutFormProps } from 'apps/user-ui/src/types';
...
const CheckoutForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Collect customer info from session/cart or add form fields
    ...
    <div className="flex justify-center items-center min-h-[80vh] px-4 my-10">
      ...
      >
        ...
        <div className="bg-gray-100 p-4 rounded-md text-sm text-gray-700 space-y-6">
          ...
          <div className="flex justify-between font-semibold pt-2 border-t border-t-gray-300">
            {coupon && coupon?.discountAmount !== 0 && (
              <>
                <span>Discount</span>
                <span className="text-green-600">
                  ₹ {coupon?.discountAmount?.toFixed(2) ?? 0}
                </span>
              </>
            )}
          </div>
